...
def run(params, config, capture, detector, reid): 
    ...
    while thread_body.process:
        ...
        # test begin
        with lock:
            outputFrame = vis.copy()
        # test end
        
        presenter.drawGraphs(vis)
        temp_1 = np.zeros((1080, 240, 3), dtype=np.uint8)
        temp_2 = np.hstack((temp_1, vis, temp_1))
        if not params.no_show:
            cv.imshow(win_name, temp_2)
        ...
        if frames_read and not set_output_params:
            ...
            if len(params.output_video):
                ...
                video_output_size = (CV_WND_PROP_FULLSCREEN, CV_WINDOW_FULLSCREEN)
                print(video_ouput_size)
...
def main():
    ...
    parser.add_argument('--device_1', type=str, default='CPU')
    parser.add_argument('--device_2', type=str, default='CPU')
    ...
    parser.add_argument('--to_web', help='Optional. Serve output to web via Flask', action='store_true')

    args = parser.parse_args()

    args.input = ['http://pi:raspberry@192.168.12.150:8090/stream.mjpg']
    args.m_detector = r'/opt/intel/openvino_2021/deployment_tools/open_model_zoo/tools/downloader/intel/person-detection-retail-0013/FP16/person-detection-retail-0013.xml'
    args.m_reid = r'/home/ubuntu/qmh/models/ir/resnet50_market.xml'
    args.config = r'configs/person.py'
    # args.history_file = r'/home/ubuntu/Documents/demo_reid/history_file'
    # args.save_detections = r'/home/ubuntu/Documents/demo_reid/detections'
    ...
    if args.detections:
        ...
        object_detector = MaskRCNN(ie, args.m_segmentation,
                                   ...
                                   args.t_segmentation,
                                   args.device_1, args.cpu_extension,
    ...
    if args.m_reid:
        object_recognizer = VectorCNN(ie, args.m_reid, args.device_2, args.cpu_extension)
    else:
        object_recognizer = None
    
    # test begin
    if args.to_web:
        t = threading.Thread(target = run, args = (args, config, capture, object_detector, object_recognizer))
        t.daemon = True
        t.start()
        app.run(host='0.0.0.0', threaded=True, debug=True, port="8091", use_reloader=False)
    else:
